<script>
    ...
    function selectAnswer(answer){
        ...
        // if(!canMarkQuestion(testSection,question.sequence)){
        //     alert("Sorry cannot mark question");
        //     return;
        // }
        ...
        {#if question}
            {#if marking}
                <div class="p-2 bg-blue-900 shadow-xl flex align-middle justify-center rounded-xl ">
                    ...
                    <p class="text-white text-right text-xs grow">{question.subject.name}</p><br>
                    <p class="text-white text-right text-xs grow">Section:{testSection.name}</p>
                </div>
                <p class="text-black text-right text-xs grow">Type:{testSection.section_type}</p>
            {/if}
            <!-- w-full  w-full -->
            ...
            {#if question.comprehension}
                <div class="text-sm leading-5 text-blue-900 overflow-scroll w-full  md:px-5 py-2">{@html atob(question.comprehension.content)}</div>
            {/if}
            <div class="text-sm leading-5 text-blue-900 overflow-scroll w-full flex p-5 m-2 rounded shadow {selectedAnswer.includes("A")?'bg-green-400':''}" on:click={()=>selectAnswer("A")}><span class=" md:px-5 bg-grey-400 rounded">A)</span>{@html atob(question.options[0].option_value)}</div>
            <div class="text-sm leading-5 text-blue-900 overflow-scroll w-full flex p-5 m-2 rounded shadow {selectedAnswer.includes("B")?'bg-green-400':''}" on:click={()=>selectAnswer("B")}><span class=" md:px-5 bg-grey-400 rounded">B)</span>{@html atob(question.options[1].option_value)}</div>                    
            <div class="text-sm leading-5 text-blue-900 overflow-scroll w-full flex p-5 m-2 rounded shadow {selectedAnswer.includes("C")?'bg-green-400':''}" on:click={()=>selectAnswer("C")}><span class=" md:px-5 bg-grey-400 rounded">C)</span>{@html atob(question.options[2].option_value)}</div>
            <div class="text-sm leading-5 text-blue-900 overflow-scroll w-full flex p-5 m-2 rounded shadow {selectedAnswer.includes("D")?'bg-green-400':''}" on:click={()=>selectAnswer("D")}><span class=" md:px-5 bg-grey-400 rounded">D)</span>{@html atob(question.options[3].option_value)}</div>
