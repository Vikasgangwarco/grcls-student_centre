<script>
 ...
 const res = await fetch(
   loginPath + "/user/students/batches/subject_get_chapter/" + batchSubjectId,
   {
     mode: "cors",
     method: "get",
 ...
 if (res.status == 200) {
   try {
     ...
     subject_chapters = response.data;
     console.log("subject_chapters");
     console.log(subject_chapters);
     console.log("subject_chapters");
   } catch (e) {
     console.log("caught");
...
<div class="mt-16 bg-white ">
  <div class="ml-2">
    <div class="text-3xl">Chapters</div>
    <div class="align-middle inline-block min-w-full overflow-hidden bg-white mt-8   rounded-bl-lg rounded-br-lg">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
            <!-- CARD 1 -->
            {#each subject_chapters as subject_chapter,index}
            
                <div class="rounded overflow-hidden shadow-md hover:shadow-xl flex flex-col">
                  <a href="/dpp_test_list/{subject_chapter._id}">
                    <div class="relative">
                    ...
                    <div class="px-6 py-4 mb-auto">
                        <div
                            ...
                            >  {subject_chapter.chapterId.name}</div
                        >
                       
                        {#if subject_chapter.dpp_tests }
                        <p class="text-gray-500 text-sm">
                          <span class="ml-2">{subject_chapter.dpp_tests.length} Dpp Test</span> 
                      </p>
                        {:else}
                        <p class="text-gray-500 text-sm">
                          <span class="ml-2"> No Dpp Test Available</span> 
                      </p>
                        {/if}
                        <p class="text-gray-500 text-sm">
