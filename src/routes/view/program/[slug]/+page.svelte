<script>
    ...
    export let data;
  export let programId = data.programId;
  console.log("studentID from slug:" + programId);
...
//    };
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/promotions/program_get/'+programId,{mode:'cors',method:'get',headers:{"Content-Type": "application/json",}});
		if(res.status==200){
			try{
					let response= await res.text();
					console.log(response);
					// jsonProgram = response;
