<script>
    ...
    export let data;
  export let teamId = data.teamId;
  console.log("studentID from slug:" + teamId);
...
//    };
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/promotions/team_get/'+teamId,{mode:'cors',method:'get',headers:{"Content-Type": "application/json",}});
		if(res.status==200){
			try{
			...
			<div class="bg-white p-6 rounded-lg shadow-lg">
	            ...
	            <div class="flex-shrink-0 flex justify-center ">
                    <span class="self-center text-4xl bg-white p-3 border-[1px] border-gray-300">
                      {@html decodeAndModifyImage1(team.image)}
                    </span>
                  </div>
