<script>
    ...
    export let data;
  export let testseriesId = data.testseriesId;
  console.log("studentID from slug:" + testseriesId);
...
//    };
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/promotions/testseries_get/'+testseriesId,{mode:'cors',method:'get',headers:{"Content-Type": "application/json",}});
		if(res.status==200){
			try{
					let response= await res.text();
					console.log(response);
					// jsonTestSeries = response;
					
					response= await JSON.parse(response);
					if(response.status == "success")
					{
						console.log("got testseries")
						testseries = response.data;
					...
					<div class="border-t border-gray-200 px-4 py-5 sm:p-0">
						<dl class="sm:divide-y sm:divide-gray-200">
							...
							<div class="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
								...
								<dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
									{@html atob(testseries.total_test)}
								</dd>
							</div>
							<!-- <div class="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
								<dt class="text-sm font-medium text-gray-500">
