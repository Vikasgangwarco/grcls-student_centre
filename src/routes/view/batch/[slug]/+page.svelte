<script>
  ...
  console.log("studentID from slug:" + batchId);
...
//    };
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/promotions/batch_get/'+batchId,{mode:'cors',method:'get',headers:{"Content-Type": "application/json",}});
		if(res.status==200){
			try{
