<script>
    ...
    export let data;
  export let testimonialId = data.testimonialId;
  console.log("studentID from slug:" + testimonialId);
...
//    };
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/promotions/testimonial_get/'+testimonialId,{mode:'cors',method:'get',headers:{"Content-Type": "application/json",}});
		if(res.status==200){
			try{
					let response= await res.text();
					console.log(response);
					// jsonTestimonial = response;
