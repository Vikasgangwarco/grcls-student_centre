<script>
  ...
  onMount(async () => {
    ...
    Token.set(token);
    console.log(loginPath + "/auth/whoami112");
    const res = await fetch(
      loginPath + "/user/students/dpp_test_state/" + testStateId,
      {
        mode: "cors",
        method: "get",
  ...
  function initializeTestState(testState) {
    ...
    for (var section in testSections) {
      for (var question in testSections[section].questions) {
        ...
        console.log("initializing test state2");
        let data = {
          question: {
        ...
        console.log("initializing test state3");
        if (
  ...
  async function syncServer() {
    ...
    const res = await fetch(
      loginPath + "/user/students/update_dpp_test_state/" + testStateId,
      {
        mode: "cors",
        method: "post",
        headers: {
  ...
  async function submitTest() {
    ...
    console.log(loginPath + "/auth/whoami");
    const res = await fetch(
      loginPath + "/user/students/test_submit_dpp/" + testStateId,
      {
        mode: "cors",
        method: "post",
        headers: {
    ...
    if (res.status == 200) {
      try {
        ...
        if (response.status == "success") {
          console.log("saved test state successful");
          window.location.href = "/results/dpp_test/" + response.data;
          return true;
        } else {
          console.log(response.message);
          return false;
