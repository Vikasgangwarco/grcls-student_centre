<script>
  ...
  console.log("verificationId from slug:" + test_id);
  ...
  onMount(async () => {
    ...
    const res = await fetch(loginPath + "/user/students/test_report/" + test_id, {
      mode: "cors",
      method: "get",
...
<body class="">
  <header>
    <nav class="navbar bg-white shadow-2xl">
      ...
      <ul class=" pr-20">
        ...
        <li>
          <a href="#services"><span class="text-gray-800">Services</span></a>
        </li>
  ...
  <div class="mt-16 p-12">
    <div class="mt-4 p-4 border-2 bg-slate-700">
      <p class=" text-white">Batch S1</p>
    </div>
    ...
    <div class="mt-4 p-4 space-y-1">
      ...
      <div class="content">
        <table class="">
          {#each batch as batch, index}
            <tr>
              <button on:click={testReport}
                class="bg-green-600 border-[1px] w-[900px] py-2 px-6 rounded-lg"
                ><div class="flex flex-row">
                  <div class="flex flex-col flex-1">
                    <div class="text-white">{batch.tests[index].name}</div>

                    <div class="text-white">{batch.tests[index].test_type}</div>
                  </div>
                  <div class="mt-3">
                    <i class="text-white fa-solid fa-circle-arrow-right" />
                  </div>
                </div></button
              >
            </tr><br />
